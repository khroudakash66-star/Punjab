...
import { Home, Plus, User, Heart, MessageCircle, Send } from "lucide-react";
...
export default function App() {
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-sm bg-neutral-900 p-6 rounded-2xl border border-neutral-800 text-center">
          <h1 className="text-2xl font-black text-amber-400 mb-1">PUNJAB</h1>
          <p className="text-xs text-neutral-400 mb-4">ਸਾਡਾ ਪੰਜਾਬ - ਸੋਸ਼ਲ ਮੀਡੀਆ</p>
          <form onSubmit={(e) => {
            e.preventDefault();
            const val = e.target.username.value.trim();
            if (val) {
              localStorage.setItem('punjab_user', val);
              setUser(val);
            }
          }} className="space-y-3">
            <input name="username" placeholder="ਆਪਣਾ ਨਾਮ / ID ਲਿਖੋ..." required className="w-full bg-neutral-800 p-3 rounded-xl text-xs text-white outline-none border border-neutral-700" />
            <button className="w-full bg-amber-500 text-black font-bold p-3 rounded-xl text-xs">ਸ਼ੁਰੂ ਕਰੋ</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans max-w-md mx-auto relative pb-20 border-x border-neutral-900">
      <header className="sticky top-0 z-50 bg-black border-b border-neutral-800 px-4 h-14 flex items-center justify-between">
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-black border-t border-neutral-800 flex justify-around py-3 z-50">
        <button onClick={() => setScreen("home")} className={screen === "home" ? "text-amber-400" : "text-white/50"}><Home size={24} /></button>
        <button onClick={() => setScreen("create")} className={screen === "create" ? "text-amber-400" : "text-white/50"}><Plus size={26} /></button>
        <button onClick={() => setScreen("profile")} className={screen === "profile" ? "text-amber-400" : "text-white/50"}><User size={24} /></button>
      </nav>
    </div>
  );
}
      ...
      {posts.length === 0 ? (
        ...
        posts.map(post => (
          <article key={post.id} className="bg-black border-b border-neutral-900">
            <div className="p-3 text-xs font-bold text-amber-400">@{post.author || "user"}</div>
            ...
            <div className="p-3 text-xs">
              <div className="flex gap-4 mb-2">
                <button onClick={() => setLikes(l => ({ ...l, [post.id]: !l[post.id] }))}>
                  <Heart size={22} fill={likes[post.id] ? "#ef4444" : "none"} stroke={likes[post.id] ? "#ef4444" : "currentColor"} />
                </button>
...
function CreateScreen({ user, reload, setScreen }) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {!preview ? (
        <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-neutral-700 rounded-2xl p-12 text-center cursor-pointer bg-neutral-900">
          <Plus className="mx-auto text-amber-500 mb-2" size={32} />
          <p className="text-xs text-neutral-300">ਫ਼ੋਟੋ ਚੁਣਨ ਲਈ ਇੱਥੇ ਕਲਿੱਕ ਕਰੋ</p>
        </div>
      ) : (
      ...
      <button onClick={publish} disabled={!preview || busy} className="w-full bg-amber-500 text-black font-bold p-3 rounded-xl text-xs disabled:opacity-40">
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center gap-3 border-b border-neutral-900 pb-4">
        <div className="w-14 h-14 rounded-full bg-neutral-900 border border-amber-500 flex items-center justify-center font-bold text-amber-400 text-lg">
          {user[0]?.toUpperCase()}
        </div>
        <div>
          <h2 className="font-bold text-sm">@{user}</h2>
          <p className="text-xs text-neutral-400">ਮੇਰੀ ਪ੍ਰੋਫਾਈਲ</p>
        </div>
      </div>
